---
permalink: /
title: "About Me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

{% include base_path %}

I am a Ph.D. & M.S. Supervisor at the College of Computer, National University of Defense Technology (NUDT).
I received the Ph.D, M.S. and B.S. degrees in Computer Science from National University of Defense Technology (NUDT) in 2015, 2010 and 2008 respectively.
I worked as a research assistant at Department of Computer Science, the University of Hong Kong during Oct. 2012 to Oct. 2013. 

I am always seeking highly self-motivated students for PhD, Master by Research.

Welcome to contact me!

Email: <img src="{{ base_path }}/images/contact/email-card.png" alt="Email address" style="height: 32px;">



Research Interests
======
Parallel systems optimization for large-scale AI workloads, with a focus on large language models (LLMs), graph neural networks (GNNs), and AI for Science (AI4Science).


Publications
======
Please refer to the selected [publications]({{ base_path }}/publications/) or the full [publication](https://dblp.org/pid/143/4279.html).

<!-- News -->
<!-- ====== -->
